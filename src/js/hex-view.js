...
function hexView(buffer, userOptions) {
    ...
    const options = Object.assign({
        ...
        asHTML: false           // adds HTML linebreak rather than \n
    }, userOptions);
    ...
    while (position < length) {
        ...
        if (byteInRow >= bytesInRow) {
            if (!options.skipEmptyLines || (options.skipEmptyLines && !currentLineIsEmpty)) {
                result += pad(currentPosition.toString(positionBase), lengthOfCurrentPositionNumber) + ':' + currentLine + lb;
            }
            byteInRow = 0;
            currentLine = '';
